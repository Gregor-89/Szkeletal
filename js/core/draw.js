...
export function draw(ctx, state, ui, fps) {
    ...
    for (const key in batches) {
        ...
        const color = key.substring(0, key.lastIndexOf('_'));
        const alpha = parseFloat(key.substring(key.lastIndexOf('_') + 1));
        ctx.globalAlpha = alpha; ctx.fillStyle = color; ctx.beginPath();
        for (const p of list) { ctx.rect(Math.round(p.x), Math.round(p.y), p.size, p.size); }
        ctx.fill();
    }
    ...
    for (const b of bombIndicators) {
        ...
        if (b.isSiege) {
            ...
            ctx.strokeStyle = `rgba(255, 255, 255, ${opacity})`; ctx.lineWidth = 3; ctx.setLineDash([10, 5]);
    ...
    if (devSettings.debugHitboxes) {
        ctx.save(); ctx.lineWidth = 1; ctx.strokeStyle = '#00FF00'; ctx.beginPath(); ctx.arc(player.x, player.y, player.size * 0.5, 0, Math.PI * 2); ctx.stroke();
    ...
    ctx.font = 'bold 14px Arial'; ctx.textAlign = 'center'; ctx.shadowBlur = 4;
