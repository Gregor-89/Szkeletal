// ==============
// GAMEDATA.JS (v1.33 - Hut HP, Nova Balance & Map Density Fix)
...
export const MAP_CONFIG = {
  ...
  OBSTACLE_STATS: {
    ...
    hut: {
      ...
      shadowOffsetY: 12,
      hp: 100, // ZMNIEJSZONO HP do 100 zgodnie ze zgłoszeniem
      isSolid: true, dropChance: 0.4, canRotate: false
    },
    water: {
...
export const PERK_CONFIG = {
  ...
  // FIX: Zmniejszono dmg oraz przebicie (pierce) dla Nova na start (level/3)
  nova: {
    max: 6,
    calculateDamage: (level) => (8 + level * 2),
    calculateCooldown: (level) => Math.max(0.5, 3.0 - (level * 0.4)),
    calculateCount: (level) => 2 + level,
    calculatePierce: (level) => Math.floor(level / 3)
  },
