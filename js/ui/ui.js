// ==============
// UI.JS (v1.11h - Layout Restore & Full Code Restoration)
...
export function updateEnemyCounter(game, enemies) { Hud.updateEnemyCounter(game, enemies); }
export function updateUI(game, player, settings, weapons, enemies = []) { Hud.updateUI(game, player, settings); }
...
export function showMenu(game, resetAllFn, uiData, allowContinue = false) {
    ...
    if (!allowContinue) { 
        if (resetAllFn) resetAllFn();
        else resetAll(uiData.canvas, uiData.settings, uiData.perkLevels, uiData, uiData.camera); 
        uiData.savedGameState = null; 
    }
    
    if(btnContinue) btnContinue.style.display = (uiData.savedGameState && allowContinue) ? 'block' : 'none';
...
export function startRun(game, resetAllFn, uiData) {
    LeaderboardUI.setGameRef(game);
    if (devSettings.presetLoaded && !devSettings.justStartedFromMenu) retryLastScenario(); 
    ...
    uiData.startTime = currentTime - devStartTime * 1000;
    uiData.lastTime = currentTime;
    
    uiData.settings.lastElite = game.time;
    uiData.settings.lastSiegeEvent = game.time; 
    if (uiData.settings.currentSiegeInterval < devStartTime) uiData.settings.currentSiegeInterval = devStartTime + 10.0; 
    
    if (LeaderboardService && LeaderboardService.trackStat) LeaderboardService.trackStat('games_played', 1);

    const tutorialSeen = localStorage.getItem('szkeletal_tutorial_seen');
    if (!tutorialSeen) {
        const overlay = document.getElementById('tutorialOverlay');
        if (overlay) { overlay.style.display = 'flex'; Menus.updateStaticTranslations(); game.paused = true; }
...
export function resetAll(canvas, settings, perkLevels, uiData, camera) {
    ...
    if (devSettings.presetLoaded === false) {
        game.score = 0; game.level = 1; game.health = PLAYER_CONFIG.INITIAL_HEALTH; game.maxHealth = PLAYER_CONFIG.INITIAL_HEALTH; 
        game.time = 0; game.xp = 0; game.xpNeeded = GAME_CONFIG.INITIAL_XP_NEEDED; game.pickupRange = PLAYER_CONFIG.INITIAL_PICKUP_RANGE;
        Object.assign(settings, { 
            spawn: GAME_CONFIG.INITIAL_SPAWN_RATE, maxEnemies: GAME_CONFIG.MAX_ENEMIES, 
            eliteInterval: GAME_CONFIG.ELITE_SPAWN_INTERVAL, lastHazardSpawn: 0, lastSiegeEvent: 0, 
        ...
        game.totalKills = 0; game.gameplayQuoteTimer = 60; 
        ...
        game.score = 0; settings.lastFire = 0; settings.lastElite = 0; 
        settings.lastHazardSpawn = 0; settings.lastSiegeEvent = 0; 
    ...
    game.magnet = false; game.magnetT = 0; game.shield = false; game.shieldT = 0; 
    game.speedT = 0; game.freezeT = 0; game.shakeT = 0; game.shakeMag = 0; 
    game.manualPause = false; game.collisionSlowdown = 0; 
    game.dynamicEnemyLimit = GAME_CONFIG.INITIAL_MAX_ENEMIES;
    game.hunger = GAME_CONFIG.MAX_HUNGER || 100;
    uiData.enemies.length = 0; uiData.chests.length = 0; uiData.pickups.length = 0; 
    uiData.bombIndicators.length = 0; uiData.hazards.length = 0; 
    if (uiData.siegeSpawnQueue) uiData.siegeSpawnQueue.length = 0;
    if (uiData.obstacles) { uiData.obstacles.length = 0; generateMap(uiData.obstacles, uiData.player); }
...
export function pauseGame(game, settings, weapons, player) {
    if (game.isDying || game.paused || game.inMenu) return;
    game.manualPause = true; game.paused = true;
    if(pauseOverlay) pauseOverlay.style.display = 'flex'; 
    if(resumeOverlay) resumeOverlay.style.display = 'none';
    try { if (statsDisplayPause) updateStatsUI(game, player, settings, weapons, statsDisplayPause); } catch (e) { console.error(e); }
    Menus.updateStaticTranslations(); 
}
...
export function resumeGame(game, timerDuration = UI_CONFIG.RESUME_TIMER) {
    ...
    if (timerDuration <= 0) { 
        if(resumeOverlay) resumeOverlay.style.display = 'none'; 
        game.paused = false; playSound('MusicGameplay'); return; 
    }
    let t = timerDuration;
    ...
    const id = setInterval(() => {
        ...
        if (t <= 0) { 
            clearInterval(id); if(resumeOverlay) resumeOverlay.style.display = 'none'; 
            game.paused = false; playSound('MusicGameplay'); 
        }
    }, 50);
}
...
export function gameOver(game, uiData) {
    ...
    const currentRun = { score: game.score, level: game.level, time: finalTimeValue, kills: game.totalKills || 0, date: new Date().toISOString() };
    ...
    if (quoteEl) {
        ...
        quoteEl.innerText = getLang(`quote_gameover_${rnd}`) || "";
    }
    saveScore(currentRun, localStorage.getItem('szkeletal_player_nick') || "GRACZ"); 
    playSound('MusicMenu');
    const btnSubmit = document.getElementById('btnSubmitScore');
    if(btnSubmit) btnSubmit.style.display = (game.score > 0) ? 'inline-block' : 'none';
    attachClearScoresListeners();
    ...
    Hud.resetHealthBarVisuals();
    Menus.updateStaticTranslations(); 
    if (LeaderboardService && LeaderboardService.trackStat) LeaderboardService.trackStat('total_playtime_seconds', finalTimeValue);
    if (LeaderboardService && LeaderboardService.syncSessionStats) LeaderboardService.syncSessionStats();
}
