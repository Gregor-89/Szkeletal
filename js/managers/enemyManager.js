// ==============
// ENEMYMANAGER.JS (v0.111b - SnakeEater Cooldown Fix)
...
export function spawnEnemy(enemies, game, canvas, enemyIdCounter, camera) {
    ...
    // FIX: Szybszy start - mniejszy margines spawnu na początku gry
    let margin = 600 / zoom;
    if (game.time < 5.0) margin = 150 / zoom;
...
export function spawnElite(enemies, game, canvas, enemyIdCounter, camera) {
    ...
    if (!bossType) {
        ...
        const availableBosses = BOSS_TYPES.filter(type => {
            if (activeBossTypes.includes(type)) return false;

            // FIX v0.111b: Wężojad (snakeEater) ma 4-minutowy cooldown po śmierci (Punkt dodatkowy Etap 1)
            if (type === 'snakeEater' && game.lastSnakeEaterDeathTime !== undefined) {
                if (game.time - game.lastSnakeEaterDeathTime < 240) return false;
            }
...
export function killEnemy(idx, e, game, settings, enemies, particlePool, gemsPool, pickups, enemyIdCounter, chests, fromOrbital = false, preventDrops = false) {
    ...
    if (!preventDrops) {
        // PUNKTACJA: Każdy zabity przeciwnik daje punkty zgodnie z jego statystykami
        ...
        // FIX v0.111b: Rejestrowanie czasu śmierci Wężojada dla cooldownu
        if (e.type === 'snakeEater') {
            game.lastSnakeEaterDeathTime = game.time;
        }
