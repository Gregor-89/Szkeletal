...
- **Obsługa Klawiatury (Full Keyboard Support):** Zaimplementowano pełne sterowanie grą bez myszki.
  - Menu: Strzałki/WSAD do nawigacji, Enter/Spacja do akceptacji, Esc/Backspace do cofania.
  - Gra: Poruszanie postacią za pomocą strzałek (klawisze Arrow Keys), działające równolegle z WSAD.
- **Assets:** Dodano unikalną pixel-artową ikonę strony (`favicon.png`) przedstawiającą wampira/czaszkę.

### Naprawiono
- **System Wznawiania Gry:** Przycisk "Kontynuuj" w menu głównym poprawnie uruchamia teraz sekwencję odliczania "PRZYGOTUJ SIĘ", zamiast wrzucać gracza prosto pod topór.
- **Flagi Językowe:** Zastąpiono systemowe emoji (psujące się na Windows) grafikami SVG, zapewniając spójny wygląd flag (PL, EN, RO) na każdym systemie.
- **Leaderboards UI:** Usunięto zbędne strzałki sortowania (⇅), pozostawiając czysty interfejs.

### Zmieniono
- **Input System:** Zintegrowano obsługę klawiatury i gamepada w jeden spójny system sterowania interfejsem (`menuNavigation.js`).

---

## [0.113] - 2026-01-07
### Dodano
- **Kapliczki (Shrines):** Wprowadzono system wizualnej sygnalizacji gotowości (Glow). Kapliczki teraz pulsują złotą poświatą, gdy leczenie jest dostępne.

### Naprawiono
- **Kapliczki:** Naprawiono mechanikę interakcji i leczenia. Zwiększono promień wykrywania gracza w `checkCollisions`, aby aktywacja była płynniejsza i niezawodna.
- **System Hazardów:** Przywrócono pełną logikę wpływu bagien na przeciwników (obrażenia, spowolnienie), gemy oraz ulepszenia (proces znikania/decay). 
- **Integracja isMega:** Przywrócono obsługę flagi `inMegaHazard` dla gracza i przeciwników, umożliwiając poprawne działanie dużych pól skażenia.
- **Fizyka Walki:** Zweryfikowano system StrikeID dla broni Nova i Orbital – obrażenia są zadawane każdej encji maksymalnie raz na sekundę, zgodnie z wymaganiami balansu.
- **Chatki:** Potwierdzono poprawne niszczenie pocisków (np. AutoGun) na chatkach w przypadku braku cechy przebicia (pierce).
- **Leaderboards**: Usunięto nadmiarowe strzałki sortowania `⇅` z nagłówków tabel, pozostawiając jedynie aktywne wskaźniki `▲▼`.
- **Leaderboards**: Naprawiono błąd synchronizacji fokusu – kliknięcie dotykiem/myszą w zakładkę poprawnie przenosi teraz żółtą poświatę systemową.
- **UI**: Skorygowano marginesy zakładek w tabeli wyników oraz przycisków ulepszeń, eliminując ucinanie poświaty i podkreśleń na krawędziach.
- **Nawigacja**: Stabilizacja pozycji flag językowych w menu głównym – dzięki zmianie pozycjonowania na stałe przestały one "przeskakiwać" podczas przełączania widoków.
- **Nawigacja**: Usunięto białą ramkę pojawiającą się na flagach po ich zaznaczeniu gamepadem.
- **Gamepad**: Naprawiono logikę przycisku (B) Back w menu głównym – teraz poprawnie chowa menu i wywołuje procedurę wznowienia gry (odliczanie).
- **Gamepad**: Dodano pełną obsługę przycisku "Postaw kawkę" (linków <a>) w menu Hot Coffee.
- **Hot Coffee**: Zmieniono domyślny fokus po wejściu do menu na przycisk "Powrót", aby zapobiec natychmiastowemu przewijaniu widoku na sam dół.
- **VFX**: Zsynchronizowano skalę pulsowania przycisku kawy z jego rozmiarem po zaznaczeniu, co wyeliminowało efekt "skakania" szerokości.

### Dodano
- **Network**: Przycisk "PONÓW PRÓBĘ" w sekcji wspierających, umożliwiający ponowne pobranie danych z Suppi.pl bez konieczności restartowania całej gry.

### Zmieniono
- **Optymalizacja**: Wprowadzono `touch-action: manipulation` dla wszystkich przycisków, co usuwa systemowe opóźnienie (ok. 300ms) kliknięcia na urządzeniach mobilnych.
- **Technical**: Przebudowano funkcję `handleMenuBack`, aby jawnie wymuszała zmianę stanu pauzy w silniku gry.

---

## [0.112] - 2026-01-07
### Naprawiono
- **System Sklepu:** Naprawiono logikę "Bicia Salda". Saldo punktów w portfelu jest teraz nadpisywane tylko, gdy wynik rundy jest wyższy niż aktualny stan posiadania.
- **Błąd Portfela:** Naprawiono błąd wyświetlania "0" punktów w interfejsie sklepu poprzez wymuszenie odświeżania salda przy przełączaniu zakładek.
- **Błąd Inicjalizacji:** Naprawiono krytyczny błąd `SIEGE_EVENT_CONFIG is not defined` w module `gameState.js`.
- **Nawigacja:** Naprawiono blokadę sterowania gamepadem/klawiaturą poprzez poprawne wywołanie `onAssetsReady` w sekwencji startowej.

### Zmieniono
- **Refaktoryzacja:** Podzielono gigantyczne pliki `main.js` oraz `menus.js` na mniejsze, logiczne moduły (`gameState.js`, `gameLoop.js`, `appLauncher.js`, `menuTranslations.js`, `menuNavigation.js`).
- **Przewodnik:** Przywrócono pełną bibliotekę opisów encji w menu Przewodnika (wszystkie 25 wpisów).""
